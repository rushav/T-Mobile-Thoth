...
All system prompts used by Project Thoth. Keep these in `backend/agents/prompts.py` as string constants.

## Thoth Classifier Prompt
Used when a user asks a question. Thoth determines which subject agent should handle it.
...
## SME Agent System Prompt
Each subject-specific agent gets this prompt, scoped to its domain.
...
## Interviewer Prompt
Thoth uses this when conducting an SME interview.
...
Previous messages in this interview:
{conversation_history}
"""
```

## Synthesis Prompt
After an interview, Thoth generates a structured summary for SME review.
...
When Thoth detects ambiguity between subjects.
...
Keep it to one short question. Be friendly.
"""
```
